...
export function renderLuaAddon(binds: ExportBind[], addonName: string, decor?: AddonDecor): string {
  ...
  return [
    ...
    `  return name and string.match(name, "^KO%d%d$") ~= nil`,
    `end`,
    ``,
    ...
    `    pcall(C_SpellBook.PickupSpellBookItem, spellId)`,
    `    if cursorReady() then return true end`,
    `    ClearCursor()`,
    `  end`,
    `  if PickupSpellBookItem then`,
    `    pcall(PickupSpellBookItem, spellId)`,
    `    if cursorReady() then return true end`,
    `    ClearCursor()`,
    `    if name then`,
    `      pcall(PickupSpellBookItem, name)`,
    `      if cursorReady() then return true end`,
    `      ClearCursor()`,
    `    end`,
    `  end`,
    ...
    `end`,
    ``,
    `local function placeVerified(slot, label)`,
    `  PlaceAction(slot)`,
    `  ClearCursor()`,
    `  if label then`,
    `    local actionType, id = GetActionInfo(slot)`,
    ...
    `  ClearCursor()`,
    `  PickupMacro(label)`,
    `  if not cursorReady() then return false end`,
    `  return placeVerified(target.slot, label)`,
    `end`,
    ``,
    `local function bindPlacedTarget(bind, target, usedToggles)`,
    ...
    `            cursor = cursor + 1`,
    `            placed = placed + 1`,
    `            bindPlacedTarget(bind, target, usedToggles)`,
    `          else`,
    ...
    `    end`,
    `    if anyHidden then`,
    `      print("|cff7c78ff" .. ADDON .. "|r: " .. TEXT.enableBarsHint)`,
    `    end`,
    `  end`,
    `end`,
    ``,
